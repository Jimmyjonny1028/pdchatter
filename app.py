...
async def signup(data: dict = Body(...)):
    ...
    if not u or not p: raise HTTPException(400, "Missing credentials")
    ref = users_collection.document(u)
    if ref.get().exists: raise HTTPException(400, "User exists")
...
async def login(data: dict = Body(...)):
    ...
    ref = users_collection.document(u)
    doc = ref.get()
    if not doc.exists: raise HTTPException(401, "Invalid login")
...
async def get_chats(current_user: str = Depends(get_current_user)):
    ...
    for doc in chats_collection.where("userId", "==", current_user).stream():
        ...
        chats.append({
            ...
            "timestamp": d.get("timestamp"), "type": d.get("type"),
